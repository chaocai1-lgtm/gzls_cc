...
try:
    ...
    with driver.session() as session:
        result = session.run("""
            MATCH (s:mfx_Student {student_id: 'test_debug'})-[:PERFORMED]->(a:mfx_Activity)
        ...
        # 清理测试数据
        session.run("MATCH (s:mfx_Student {student_id: 'test_debug'})-[r:PERFORMED]->(a:mfx_Activity) DELETE r, a")
        session.run("MATCH (s:mfx_Student {student_id: 'test_debug'}) DELETE s")
        print("   ✅ 测试数据已清理")
except Exception as e:
    print(f"   ❌ 测试失败: {e}")
...
try:
    with driver.session() as session:
        for module in modules:
            result = session.run("""
                MATCH (a:mfx_Activity {module: $module})
                RETURN count(a) as count
            """, module=module)
